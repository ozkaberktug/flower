...
public class Project {
    ...
    public void undo() {
        if (historyIndex == 0) {
            App.getInstance().statusPanel.getController().setStatus("Reached to the first state", StatusPanelController.ERROR);
            return;
        }
    ...
    public void redo() {
        if (historyIndex == history.size()) {
            App.getInstance().statusPanel.getController().setStatus("Reached to the last state", StatusPanelController.ERROR);
            return;
        }
